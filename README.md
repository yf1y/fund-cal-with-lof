...
| 功能 | 你可以看到什么 |
| --- | --- |
| **LOF 看板** | 21 只基金的实时估值、场内价格、溢价率、已估持仓比例和估值说明；可展开查看基金构成 |
| **基金搜索** | 输入完整 6 位基金代码或完整基金名称，按最新披露持仓计算估算净值并查看单项贡献 |
...
4. 点击“查看构成”，可以查看每项持仓的权重、人民币涨跌、对基金净值的贡献、计价方式以及汇率时间。

构成明细会同时列出已披露、已估和未披露或未估的比例。当前口径只把成功估值的披露持仓按真实权重计入；其余部分按 0 收益处理，不会把前十大持仓的涨跌放大到整个基金。
