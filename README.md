...
| 页面 | 能做什么 |
| --- | --- |
| **LOF 看板** | 浏览全量目录，按代码、名称或行业筛选；21 只深度维护基金固定置顶，其次是已有持仓的基金；按需计算结果会保存并在刷新后自动恢复 |
...
- 看板每 5 分钟自动发起一次更新；成功取得的汇率在服务端最多缓存 1 分钟，页头显示本轮实际使用的汇率时间。
...
- `lof_estimates`：保存按需计算过的 LOF，刷新页面后继续展示。
